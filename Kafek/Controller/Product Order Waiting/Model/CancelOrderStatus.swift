...
struct Api_ChangeOrderStatus : Codable {
    let result : Res_CancelOrderStatus?
    let message : String?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent(Res_CancelOrderStatus.self, forKey: .result)
        message = try values.decodeIfPresent(String.self, forKey: .message)
        status = try values.decodeIfPresent(String.self, forKey: .status)
    }
}

struct Res_CancelOrderStatus : Codable {
    let id : String?
    let user_id : String?
