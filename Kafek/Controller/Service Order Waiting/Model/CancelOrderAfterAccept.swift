//
//  CancelOrderAfterAccept.swift
//  Kafek
//
//  Created by Techimmense Software Solutions on 20/02/25.
//

import Foundation

struct Api_WorkForThreeModule : Codable {
    let result : Res_WorkForThreeModule?
    let message : String?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent(Res_WorkForThreeModule.self, forKey: .result)
        message = try values.decodeIfPresent(String.self, forKey: .message)
        status = try values.decodeIfPresent(String.self, forKey: .status)
    }
}

struct Res_WorkForThreeModule : Codable {
    let id : String?
    let user_id : String?
